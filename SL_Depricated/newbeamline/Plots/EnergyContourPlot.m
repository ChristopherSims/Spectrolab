...
subaxis(2,6,6+ii); 
C = imread(strcat('cont\',num2str(ii),'.png'));
imshow(C,[], 'XData', [-1 1]...
...
% for ii = 1:6
% subaxis(2,6,6+ii,'sv',0.01,'sh',0.01); 
% f = squeeze(B(:,Energy_Contour(ii),:))';
% f = imresize(f,50);
% h_Sobel = fspecial ('sobel');
% f_y = imfilter ( double(f) , h_Sobel , 'replicate') ;
% imshow(flipud(f_y),[], 'XData', [KX_Angle_Min KX_Angle_Max]...
%     , 'YData', [KY_Angle_Min KY_Angle_Max], 'colormap', bone);
% xlabel(strcat('K_y','(1/',Ang,')'),'FontSize',12)
% text(KX_Angle_Min + 0.10,KY_Angle_Max - 0.2,...
%     strcat('-',num2str(round(round(abs(eV(Energy_Contour(ii)) - eV(ypos))*1000.0)...
%     /100)*100),'meV') ...
